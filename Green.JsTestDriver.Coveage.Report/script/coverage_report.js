/**
 * Created by JetBrains RubyMine. User: whitewolf Date: 12-9-22 Time: 上午9:41 To
...
var coverage_report = function($scope, $window) {
	...
	$scope.totalRate = $window.totalRate;
	$scope.coverageLimteRate = $window.coverageLimteRate;
	$scope.fileUrlParam = new Uri($window.location.href).QueryParam("file",
			null);
	$scope.isTotalCoverage = $scope.fileUrlParam == null;

	if (!$scope.isTotalCoverage) {
		$scope.currentFile = underscore.filter($scope.coverageData, function(
				file) {
	...
	$scope.getTotalStatus = function() {
		return this.totalRate >= this.coverageLimteRate ? "success" : "error"
	};
}
